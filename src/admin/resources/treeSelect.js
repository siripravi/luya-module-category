...
zaa.factory("linker", function () {
  ...
  return function (arr, key) {
    ...
    arr.forEach(function (newItem, index) {
      ...
      if (!angular.equals(oldItem, newItem)) link[index] = newItem;
    });
...
zaa.directive("treeSelect", [
  "$compile",
  "$filter",
  function ($compile, $filter, $sanitize) {
    ...
    return {
      ...
      controller: [
        ...
        "$filter",
        "AdminToastService",
        function ($scope, $http, $filter, AdminToastService) {
          $scope.toggleExpand = "";
          ...
          $scope.initCheckbox = function (item, parentItem) {
            $scope.model[item.key] =
              (parentItem && $scope.model[parentItem.key]) ||
              $scope.model[item.key] ||
              false;
          ...
          return $scope.$on("changeParent", function (event, parentScope) {
            ...
            $scope.model[parentScope.item.key] =
              $filter("selected")(children, $scope.model).length ===
              children.length;
