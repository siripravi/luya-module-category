...
zaa.filter('selected', ['$filter', function ($filter) {
        return function (input) {
            ...
            if( input[key] && (el !== null) && (el.innerText !== "")){
                text = el.innerText;
                results.push(text);  //({id:key,name:text});
    ...
    return function (input,items) {
      ...
      /*  angular.forEach(items, function(item) {
          ...
          // alert(copy.key);
           ...
           if(copy.children){
            $filter('objToArray')(input,copy.children);
            
           if(input[Number(copy.key)]){
           }
     });
        ...
        return {
            ...
            controller: ['$scope', '$http','$filter','AdminToastService', function ($scope, $http,$filter,AdminToastService) {
               ...
               // $scope.categories = {};  
                ...
                $scope.getTreeData = function (id) {                        
                    $http.get('admin/api-category-category/tree?id=' + id).then(function (r) {
                ...
                $scope.initCheckbox = function (item, parentItem) {
                   ...
                   //  $scope.model.names[item.key] = item.title;//.push({"id":item.key, "name":item.key});
            ...
            $scope.$on('changeChildren', function (event, parentItem) {
              ...
              angular.forEach(parentItem.children, function (child){
                    ...
                    // $scope.model.names[child.key] = child.title;           
            ...
            return $scope.$on('changeParent', function (event, parentScope) {
              ...
              //  children = parentScope.item.children;
                ...
                // $scope.model.names[parentScope.item.key] = parentScope.item.title;
